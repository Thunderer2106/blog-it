    ...
    <div className="p-6 md:mx-auto">
      ...
      <div className="flex flex-wrap gap-6 py-6 mx-auto justify-center">
        <DataTable
          ...
          data={users}
          columns={[
            { label: 'User Image', key: 'profilePicture' },
        ...
        <DataTable
          ...
          data={comments}
          columns={[
            { label: 'Comment Content', key: 'content' },
...
function StatCard({ title, total, lastMonth, icon }) {
  return (
              ...
              <Table.Row key={index} className="bg-white dark:border-gray-700 dark:bg-gray-800">
                {columns.map((col) => (
                  <Table.Cell key={col.key}>
                    {col.key === 'profilePicture' || col.key === 'image' ? (
                      <img
                        src={item[col.key]}
                        alt={col.label}
                        className={`w-${col.key === 'profilePicture' ? '10' : '14'} h-${col.key === 'profilePicture' ? '10' : '10'} rounded${col.key === 'profilePicture' ? '-full' : '-md'} bg-gray-500`}
                      />
                    ) : (
                      <p className={col.key === 'content' ? 'line-clamp-2' : ''}>{item[col.key]}</p>
                    )}
                  </Table.Cell>
                ))}
              </Table.Row>
